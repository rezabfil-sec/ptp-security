				 ...
				 struct ptp_message *resp, tmv_t residence)
{
	...
	/* Prong A: re-calculate ICV if needed; use egress (p) */
	policy = query_security_policy(resp, clock_get_spd(p->clock));
	protect = p->protect_messages && policy;

	if (protect && protect_message(p, resp, policy)) {
			      ...
			      struct ptp_message *msg, tmv_t residence)
{
	...
	/* Prong A: re-calculate ICV if needed; use egress (p) */
	policy = query_security_policy(fup, clock_get_spd(p->clock));
	protect = p->protect_messages && policy;

	if (protect && protect_message(p, fup, policy)) {
		pr_err("%s: calculating ICV failed", p->log_name);
	}
...
int tc_forward(struct port *q, struct ptp_message *msg)
{
	...
	for (p = clock_first_port(q->clock); p; p = LIST_NEXT(p, list)) {
		...
		/* Prong A: re-calculate ICV if needed */
		policy = query_security_policy(msg, clock_get_spd(p->clock));
		protect = p->protect_messages && policy;

		if (protect && msg_type(msg) == ANNOUNCE) {
			if (protect_message(p, msg, policy)) {
				pr_err("%s: calculating ICV failed", p->log_name);
			}
		}
